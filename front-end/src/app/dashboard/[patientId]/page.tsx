...
export default async function DashboardPage({
  ...
  // Truncate to 5 symptoms
  const symptoms = clinical_brief.key_symptoms.slice(0, 5);

  // Take top 3 most significant deltas to display in UI format
  const criticalDeltas = biometric_deltas
    .filter(d => d.clinically_significant)
    .sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta))
    .slice(0, 3)
    .map(d => ({
      label: d.metric.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()).replace(/_*[a-zA-Z]*_/, ""), // basic formatting
  ...
  // Create dynamic UI risk bars based on actual risk profile data
  const riskFactors = result.risk_profile?.factors || [];

  // Helper to map severity to gradient and width multiplier
  const getSeverityStyle = (severity: string, weight: number) => {
    const w = Math.min(220, Math.max(40, weight * 2.2)); // Scale weight to pixel width (approx max 220px)
  ...
  return (
    <div className="flex h-screen w-full flex-col bg-white font-poppins">
      {/* ── Navigation Bar ── */}
          ...
          <div className="glass-card flex flex-1 flex-col overflow-hidden rounded-[24px]">
            ...
            <div className="flex items-center px-[18px]">
              <div className="nav-pill-active flex items-center gap-1.5 rounded-[14px] px-3 py-[5px]">
                ...
                <ChevronDown className="h-3 w-3 text-[var(--purple-primary)]" />
              </div>
            </div>
